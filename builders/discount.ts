...
import {buildFakeDiscountType, buildFakeDiscountValueType, buildFakeTakeawayType} from "@builders/commons";
...
export const fakeDiscount = (override: Partial<Discount> = {}): Discount =>{
    ...
    pastDate.setDate(recentDate.getDate() - faker.number.int({ min: 1, max: 30 }));

    const futureDate = new Date();
    futureDate.setDate(recentDate.getDate() + faker.number.int({ min: 1, max: 30 }));



    const days = [ ...new Set(Array.from({ length: faker.number.int({min:0, max: 7}) }, () =>
        faker.date.weekday()
    ))];
    ...
    const fakeValue = {
        ...
        products: [],
        voucherCode : `E2E test ${faker.string.octal({length:3})}`,
        usageLimit : faker.number.int({ min: 1, max: 100 }),
        singleUse : false,
        minimumOrderValue: parseFloat(faker.commerce.price({ min: 1, max: 100 })),
        ...override
    }
