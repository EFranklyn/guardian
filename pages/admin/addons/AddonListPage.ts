...
export class AddonListPage {
    ...
    constructor(page: Page) {
        ...
        this.addAddonButton =  this.page.getByRole('button').filter({ hasText: 'add' });
        this.inputFind = this.page.getByRole('textbox', { name: 'Name', exact: true });
    ...
    async selectAddonGroup(addonName: string) {
        this.addonSelected = await this.page.getByRole('row')
        .filter({ has: this.page.getByRole('cell', { name: new RegExp(`^${addonName}$`, 'i') })
        });
    }
