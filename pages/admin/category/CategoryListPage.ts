...
export class CategoryListPage {
	...
	constructor(page: Page) {
		...
		this.addCategoryButton = this.page.getByRole('button').filter({
			hasText: 'add',
		});
		this.categorySelected = this.page.getByRole('row').nth(1);
		this.confirmModalButton = this.page.getByRole('button', {
			name: 'OK',
		});
	}
	...
	async selectCategory(categoryName: string) {
		this.categorySelected = this.page.getByRole('row').filter({
