...
export class DiscountListPage {
	...
	constructor(page: Page) {
		...
		this.addDiscountButton = this.page.getByRole('button').filter({
			hasText: 'add',
		});
		this.discountSelected = this.page.getByRole('row').nth(1);
		this.inputFindVouncherCode = this.page.getByRole('textbox', {
			name: 'Voucher code',
		});
	}
	...
	async clearFind() {
		...
		const clearButton = this.inputFindVouncherCode.getByRole('button', {
			name: 'Clear',
		});
	...
	async selectDiscount(description: string) {
		this.discountSelected = this.page.getByRole('row').filter({
