...
import type { AddOn, AddOnGroup } from '../../../schemas/addon';
...
export class ProductAddonManager {
	...
	constructor(page: Page) {
		this.page = page;
		this.addAddonGroupButton = this.page
			.getByRole('toolbar')
			.filter({
				hasText: 'Options deleteinfoadd',
			})
			.getByRole('button');
		...
		this.checkboxHideAddonsName = this.page.getByRole('checkbox', {
			name: 'Hide Addons Name',
		});
		this.checkboxShowName = this.page.getByRole('checkbox', {
			name: 'Show Name',
		});
		this.checkboxDisabled = this.page
			.getByRole('checkbox', {
				name: 'Disabled',
			})
			.nth(1);
		this.addAddonButton = this.page.getByRole('button', {
			name: 'Add Addon',
		}); // resolve this desgraça
	...
	async addOnFormFill(index: number, addOn: AddOn) {
		...
		addOn.drsDeposit > 0 &&
			(await formAddOn
				...
				.getByLabel('DRS Deposit')
				.fill(addOn.drsDeposit.toString()));
	}
